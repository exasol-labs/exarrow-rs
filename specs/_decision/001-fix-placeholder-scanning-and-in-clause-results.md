...
## ADR: Investigate multi-value IN-list bug before patching

**ID:** investigate-multi-value-in-list-bug-before-patching
**Plan:** fix-placeholder-scanning-and-in-clause-results
**Status:** Accepted
...
Investigation confirmed that bug #18 does not reproduce on `main` at commit `e093648` (see the companion decision on non-reproducibility). The investigation discipline prevented an unnecessary and potentially harmful speculative patch.

## ADR: Bug #18 (multi-value IN-list zero rows over native transport) not reproducible on current main

**ID:** bug-18-multi-value-in-list-not-reproducible
**Plan:** fix-placeholder-scanning-and-in-clause-results
**Status:** Accepted

### Context

Investigation per the prior decision was run against `main` at commit `e093648` using a diagnostic integration test that creates a temp schema, inserts three rows, and runs the failing queries over the native transport. The raw wire bytes and parsed response fields were captured and inspected.
