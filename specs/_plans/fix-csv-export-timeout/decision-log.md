...
- **Direction change:** Task 3.1 bullet 4 now requires binding the timeout result to its own `let` statement so every borrow is released before the elapse handling runs, and names an immutably borrowed flag as the shape that removes the conflict outright. Round 4 replaced this round's `std::cell::Cell<bool>` with `std::sync::atomic::AtomicBool`, because `&Cell<bool>` is not `Send`; see finding [20].
- **Promotes to ADR:** no
...
### [20] [plan-review] Round 4: `Cell<bool>` would have made every export future `!Send`

- **Finding:** Round 3's fix for finding [9] named `std::cell::Cell<bool>` as the preferred shape for the progress flag. The flag is set inside the timed async block and read outside it, so the block captures `&Cell<bool>` across two await points. `&Cell<bool>` is `Send` only if `Cell<bool>` is `Sync`, which it is not. `plan-reviewer` compile-verified the failure with `rustc --edition 2021`: `error[E0277]: Cell<bool> cannot be shared between threads safely`. Every public `Connection::export_*` future would have become `!Send`, so `tokio::spawn` of a long export would stop compiling. Nothing in-repo detects it: no `tokio::spawn` appears in `tests/`, `examples/`, or `benches/`, and `src/adbc_ffi.rs` uses `block_on` throughout. It would have shipped as a silent seventh breaking change on a plan whose § Impact opens with six.
- **Direction change:** Task 3.1 bullet 4 now requires `std::sync::atomic::AtomicBool` with `store(true, Ordering::Relaxed)` inside the timed block and `load(Ordering::Relaxed)` after it, and states why `Cell` is forbidden. The flag crosses no thread boundary, so `Relaxed` is sufficient and no stronger ordering is warranted. A sixth task-3.1 requirement adds `test_export_to_callback_future_is_send`, a `--lib` compile-time check applying `fn assert_send<T: Send>(_: T) {}` to the future `export_to_callback` returns. The guarantee therefore cannot regress silently. The § Patterns row and finding [9] mirror the change, and § Verification → Scenario Coverage gains the test. The two-round review cap stopped the plan here, and the human approved the prescribed fix on `open-questions.md` with "Apply both fixes as written".
- **Promotes to ADR:** no

### [21] [plan-review] Round 4: the `is_closed()` change would have panicked two existing mock tests

- **Finding:** Task 2.3 claimed the two assertions at `src/adbc/connection.rs:3020` and `:3968` "still hold, since a live transport reports `is_connected() == true`". That is false for this crate's test double. `MockTransport` is generated by `mockall::mock!` (`src/transport/test_support.rs:18`), which panics on any call to a method with no expectation set. `grep -rn "expect_is_connected" src/` returns nothing. After task 2.3, `is_closed()` calls `is_connected()` on every live-session path, so both tests panic. `cargo test --lib` then goes red at the Group B boundary, and the Checklist's "Unit tests | 0 failures" gate cannot pass. The task framed `:3020` as something to confirm rather than change, so the implementer was told the failure would not happen. The cheapest unblock, `expect_is_connected().returning(|| false)`, satisfies both tests while inverting the new behavior permanently.
- **Direction change:** Task 2.3 now names `MockTransport` as a panic-on-unexpected-call double and lists three mandatory test updates. Both existing mocks get `transport.expect_is_connected().returning(|| true);`. A new sibling test, `is_closed_reports_true_when_the_transport_is_terminated`, drives the mock's `is_connected()` to `false` and asserts `is_closed()` returns `true` while the session is still open. That test is the only executed coverage of the OR branch. § Verification → Scenario Coverage now names it in place of the bare "(extended at `:3956`)" note. The two-round review cap stopped the plan here, and the human approved the prescribed fix on `open-questions.md` with "Apply both fixes as written".
- **Promotes to ADR:** no
