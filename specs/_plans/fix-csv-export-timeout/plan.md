...
| `AtomicBool` progress flag owned outside the timed block | `export_to_callback` | The timed future is dropped on elapse, so the conditional terminate needs the flag to survive it. `&AtomicBool` is `Send` and `&Cell<bool>` is not, so a `Cell` would make every public export future `!Send` |
...
2. Transport termination
   ...
   - [ ] 2.3 Change `Connection::is_closed()` (`src/adbc/connection.rs:1053-1055`) to `self.session.is_closed().await || !self.transport.lock().await.is_connected()`, so one fact about liveness has one answer. `MockTransport` is a `mockall::mock!` double (`src/transport/test_support.rs:18`) that panics on an unexpected call, and no test sets `expect_is_connected` today, so three test updates are mandatory:
     - Add `transport.expect_is_connected().returning(|| true);` to the mock in `connect_survives_a_schema_from_the_uri_that_does_not_exist_yet` (`src/adbc/connection.rs:3000`). Its assertion at `:3020` runs against a live session, so it reaches `is_connected()` and would otherwise panic.
     - Add the same expectation to the mock in `is_closed_reflects_the_session_state` (`:3957`). Its pre-shutdown assertion at `:3968` does not short-circuit, so it reaches `is_connected()`. The post-shutdown assertion at `:3970` short-circuits on the closed session and reaches nothing.
     - Add a sibling test `is_closed_reports_true_when_the_transport_is_terminated`, whose mock returns `false` from `is_connected()`, asserting `is_closed()` returns `true` while the session is still open. A separate test is required, because one mock instance cannot return both answers. This is the only executed coverage of the OR branch this task introduces.
3. Optional export timeout
   - [ ] 3.1 Change `CsvExportOptions::timeout_ms` to `Option<u64>` with default `None` in `src/export/csv.rs`, and rework the timeout handling in `export_to_callback` (`src/export/csv.rs:486-506`) in the same task, because `:486` and `:505` are the field's only consumers and the crate does not compile between the two changes. Keep the builder signature `timeout_ms(u64)` and wrap the argument in `Some`. Update `test_csv_export_options_default` and `test_csv_export_options_builder`. The rework must satisfy six requirements: [expert]
     ...
     - Pin the `Send` guarantee with a `--lib` unit test named `test_export_to_callback_future_is_send`. Define `fn assert_send<T: Send>(_: T) {}` and apply it to the future `export_to_callback` returns over a `MockTransport` (`src/transport/test_support.rs`), without awaiting that future. The test is a compile-time check: it fails the build if the progress flag or any other captured state makes the export future `!Send`. Every public `Connection::export_*` method awaits this future, so `tokio::spawn` of a long export depends on it.
...
| csv-export: No client-side export timeout by default | Unit | `src/export/csv.rs` | `test_export_to_callback_future_is_send` |
...
| session-and-lifecycle: Terminate a connection whose in-flight response is no longer trusted (`is_closed()`) | Unit | `src/adbc/connection.rs` | `is_closed_reports_true_when_the_transport_is_terminated` |
...
- `test_export_to_callback_future_is_send` maps to the default-timeout scenario, because an unbounded export is the case a caller runs under `tokio::spawn`. It is a compile-time check, so it proves the export future stays `Send` and asserts nothing at runtime.
