...
//!
//! ```
//! use exarrow_rs::query::export::{ExportQuery, RowSeparator, DelimitMode};
//!
...
impl ExportQuery {
    ...
    ///
    /// ```
    /// use exarrow_rs::query::export::ExportQuery;
    ///
    /// let query = ExportQuery::from_table("users")
    ///     .at_address("192.168.1.100:8080")
    ///     .build();
    ...
    /// use exarrow_rs::query::export::ExportQuery;
    ///
    /// let query = ExportQuery::from_query("SELECT * FROM users WHERE active = true")
    ///     .at_address("192.168.1.100:8080")
    ///     .build();
    /// ```
